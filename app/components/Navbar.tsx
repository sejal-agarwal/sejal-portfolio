...
export default function Navbar() {
  ...
  return (
    <>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`text-nav-link transition-opacity duration-200 ${
                    isActive
                      ? "opacity-100 active-underline-secondary font-medium"
                      : "opacity-60 hover:opacity-100 hover-underline-secondary"
                  }`}
                >
                  {link.label}
                </Link>
      ...
      <aside
        className={`fixed top-0 right-0 h-full w-4/5 max-w-sm bg-primary z-50 p-6 flex flex-col justify-between shadow-2xl transition-transform duration-300 ease-in-out lg:hidden ${
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setIsOpen(false)}
                  className={`text-nav-link text-black w-fit transition-opacity duration-200 ${
                    isActive
                      ? "opacity-100 active-underline-secondary font-medium"
                      : "opacity-60 hover:opacity-100 hover-underline-secondary"
                  }`}
